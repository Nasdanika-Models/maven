package org.nasdanika.models.maven.generator.tests;
...
import org.nasdanika.models.maven.processors.EcoreGenMavenProcessorsFactory;
 ...
 *
 */
public class TestMavenModelDocGen {
	
	@Test
	public void testGenerateMaveModelDoc() throws IOException, DiagnosticException {
		List<EPackage> ePackages = Arrays.asList(EcorePackage.eINSTANCE, MavenPackage.eINSTANCE);
		...
		URI baseActionURI = URI.createURI("local://maven.models.nasdanika.org/");
		
		Map<EPackage, URI> packageURIMap = Map.ofEntries(
			Map.entry(EcorePackage.eINSTANCE, URI.createURI("https://ecore.models.nasdanika.org/")),			
			Map.entry(MavenPackage.eINSTANCE, baseActionURI)	
		);
				...
				if (element instanceof EObjectNode) {
					...
					if (target == topLevelPackage) {
						...
						if (processor instanceof WidgetFactory) {
							...
							if (topLevelPackage == MavenPackage.eINSTANCE) { 							
								mavenProcessor = widgetFactoryNodeProcessor;
							}
						}
					}
				}
		...
		File output = new File(actionModelsDir, "maven.xmi");
		Resource actionModelResource = actionModelsResourceSet.createResource(URI.createFileURI(output.getAbsolutePath()));
		Collection<Label> labels = mavenProcessor.createLabelsSupplier().call(progressMonitor, diagnosticConsumer);
		...
		String siteMapDomain = "https://maven.models.nasdanika.org";		
